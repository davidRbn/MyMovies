           ...
           {  !isLoaded ? <Loader/> : favoris1.error ? <p style={stylesSearchMovie.textError}>{favoris1.error}</p> : favoris1.map((movie,index) =>  
                    <List key={index} classes={{}} >
                         ...
                         <ListItemText primary={<React.Fragment>
                                                ...
                                                // primary={`${movie.title}`}
                                       classes={{ root: classes.listItemText, // class name, e.g. `classes-nesting-root-x`  
                                                 }}
                                                      className='test'
                                        secondary={
                                                   <React.Fragment  >
                         ...
                         >
                           {`${movie.details}`}
                                  ...
                                  <StarIcon  className={classes.starIcon} onClick={e => deleteFavoris(e,movie.idMovie)} />
                      
                         <Typography style={stylesSearchMovie.typo}>{movie.description.length > 350 ? `${movie.description.substring(0,350)} ...`:movie.description }</Typography>
